...
<div class="card" style="margin-bottom: 20px;">
    ...
    <div style="margin-bottom: 15px;">
        <button id="btnTabManual" class="qr-tab-button active" onclick="showManualInput(); return false;">Input Manual</button>
        <button id="btnTabCamera" class="qr-tab-button" onclick="showCameraScan(); return false;">Scan dengan Kamera</button>
    </div>
    
    <!-- Input Manual -->
    <div id="manualInputSection">
        <form method="GET" action="pengembalian.php" class="modal-form" style="display: flex; gap: 10px;">
            <input type="text" name="qr_data" id="qr_data_input" placeholder="Input data QR Code manual" style="flex-grow: 1;" required>
    ...
    <div id="cameraScanSection" style="display: none;">
        ...
        <button onclick="stopCameraScan()" class="btn btn-danger" style="margin-top: 15px;"><i class="icon-x"></i> Tutup Kamera</button>
    </div>
    
    <p style="margin-top: 10px; text-align: center;">Atau gunakan tombol "Proses Kembali" dari halaman Peminjaman Aktif.</p>
</div>
...
<div class="card" style="border: 2px solid var(--secondary-color);">
    ...
    <table class="data-table" style="margin-top: 10px;">
        ...
        <tr><th>Anggota</th><td><?php echo $peminjaman_data['nama_anggota']; ?></td></tr>
        <tr><th>Buku</th><td><?php echo $peminjaman_data['judul_buku']; ?> (<?php echo $peminjaman_data['kode_buku']; ?>)</td></tr>
        <tr><th>Tanggal Pinjam</th><td><?php echo $peminjaman_data['tanggal_pinjam']; ?></td></tr>
        <tr><th>Harus Kembali</th><td><?php echo $peminjaman_data['tanggal_kembali_harus']; ?></td></tr>
        <tr><th>Tanggal Kembali Hari Ini</th><td><?php echo date('Y-m-d'); ?></td></tr>
        <tr><th>Potensi Denda</th><td>Rp <?php echo number_format(calculate_fine(date('Y-m-d'), $peminjaman_data['tanggal_kembali_harus']), 0, ',', '.'); ?></td></tr>
    </table>
    ...
    <form method="POST" action="pengembalian.php" style="margin-top: 20px;">
        ...
        <input type="hidden" name="tanggal_kembali_harus" value="<?php echo $peminjaman_data['tanggal_kembali_harus']; ?>">
        <button type="submit" class="btn btn-success" onclick="return confirm('Konfirmasi proses pengembalian buku ini?');"><i class="icon-arrow-down"></i> Selesaikan Pengembalian</button>
    </form>
</div>
<?php endif; ?>
...
function startCameraScan() {
    ...
    html5QrcodeScanner.start(
        ...
        (decodedText, decodedResult) => {
            ...
            resultContainer.innerHTML = '<div style="color: green; padding: 10px; background: #d4edda; border-radius: 5px; margin-bottom: 10px;"><strong>QR Code berhasil di-scan!</strong></div>';
        ...
        resultContainer.innerHTML = '<div style="color: red; padding: 10px; background: #f8d7da; border-radius: 5px;">Error: Tidak dapat mengakses kamera. Pastikan browser mendukung dan izin kamera sudah diberikan.</div>';
    });
}
